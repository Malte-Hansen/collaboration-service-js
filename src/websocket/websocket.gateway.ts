...
export class WebsocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
   ...
   */
  sendUserPositionsMessage(roomId: string, message: ForwardedMessage<UserPositionsMessage>): void {
    const userId = message.userId;
    const client = this.sessionService.lookupSocket(userId);
    if (client) {
      // Exclude sender of the message if it is connected to the server
      client.to(this.getVrRoom(roomId)).to(this.getSpectatingRoom(roomId)).emit(USER_POSITIONS_EVENT, message);
    } else {
      // Otherwise send to all clients
      this.server.to(this.getVrRoom(roomId)).to(this.getSpectatingRoom(roomId)).emit(USER_POSITIONS_EVENT, message);
    }
  }
