...
describe('room', () => {
    ...
    it('joining clients receive landscape', async () => {
        return new Promise<void>(async (resolve, reject) => {
            ...
            // client joins 
            const response = await request(baseURL).post('/room/' + roomId + '/lobby').send(joinLobbyPayload);
            var socket = io(baseURL, {
                transports: ["websocket"],
                query: {
                    "ticketId": response.body.ticketId,
    ...
    it('move object', async () => {
        return new Promise<void>(async (resolve, reject) => {
            ...
            client2.socket.on(OBJECT_MOVED_EVENT, (msg) => {
                // forwarded message is correct
                expect(msg).toStrictEqual({ userId: client1.id, originalMessage: moveMessage });
                resolve();
            });
